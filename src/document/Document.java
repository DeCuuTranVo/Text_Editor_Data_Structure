...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		String syllableAlphabet = "AEIOUYaeiouy";
		
		int countSyllable = 0;
		boolean countFlag = true;
		for (int i = 0; i < word.length(); i++) {
			char currentChar = word.charAt(i);
			if (syllableAlphabet.indexOf(currentChar) != -1) { // a vowel is founded
				if (countFlag == true) {
					countSyllable++;
					countFlag = false;
				}
			} else { // a consonant is founded
				countFlag = true;
			}
		}
		
//		System.out.print("Inside: " + word + "=>" + countSyllable);
		
		// if the word has a lone 'e' at the end
...
//		System.out.println(word);
		if (word.length()==1) {
			...
			if ((syllableAlphabet.indexOf(secondToLastChar)==-1) && (lastChar == 'e')) {
				// if that word has no other syllables so far -> pass;
				if (countSyllable == 1) {
					;
				} else { // if the word has more than one syllable including final lone 'e'
					countSyllable--; // (do not count that 'e' as a syllable) 
				}
			}
		} 
//		System.out.println("=>" + countSyllable);
	...
	public double getFleschScore()
	{
		...
		double numSentences = (double)this.getNumSentences();
		double numSyllables = (double)this.getNumSyllables();
