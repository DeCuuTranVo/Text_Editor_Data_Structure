...
//		System.out.println(tokens);
		...
		for (int i = 0; i < tokens.size(); i++) {
			...
			if (this.isWord(currentToken)) {
				this.numWords++;
				int numSyllableInWord = this.countSyllables(currentToken);
				this.numSyllables += numSyllableInWord;
			} else {
				this.numSentences++;
			}
		}
		
		if ((tokens.size() > 0) && this.isWord(tokens.get(tokens.size()-1))) {
			this.numSentences++;
		}
